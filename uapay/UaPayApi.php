<?php
if(!class_exists('JWTEncoderInterface')) require (VMPATH_PLUGINS.'/vmpayment/uapay/uapay/library/Jweety/JWTEncoderInterface.php');
if(!class_exists('JWTEncoder')) require VMPATH_PLUGINS .'/vmpayment/uapay/uapay/library/Jweety/JWTEncoder.php';
...
class UaPayApi
{
    ...
    function __construct($clientId,$secretKey,$testMode,$type_payment,$user_redirect_url)
    {
        $this->clientId = trim($clientId);
        $this->secretKey = trim($secretKey);
        $this->testMode = ($testMode == '1') ? 1 : 0;
        $this->type_payment = ($type_payment == '1') ? self::OPERATION_HOLD : self::OPERATION_PAY;
        $this->user_redirect_url = !empty($user_redirect_url) ? trim($user_redirect_url) : '';
